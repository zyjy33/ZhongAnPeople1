...
public class MyShopingCartllAdapter extends BaseAdapter {
	...
	// 构造器
	public MyShopingCartllAdapter(ArrayList<ShopCartData> list,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			try {
				...
				// System.out.println("list_cart_id.size()============="+list_cart_id.size());
				// System.out.println("list.size()============="+list.size());
				//
				//
				// if (list_cart_id.size() < list.size()) {
				...
				// for (int i = 0; i < list.size(); i++) {
				// String cart_id = list.get(i).getArticle_id();
	...
	private void changeReduce(int index) {
		try {
			...
			if (list.get(index).getQuantity() != 1) {
				...
				// //保留2位小数
				// double total_c_ll = c.setScale(2,
