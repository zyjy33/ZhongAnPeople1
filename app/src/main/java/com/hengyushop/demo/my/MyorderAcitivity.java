 ...
 *
 */
public class MyorderAcitivity extends FragmentActivity{
...
//		fragments.add(new CompleteyesFragment());
//		fragments.add(new ServicetheFragment());
//		fragments.add(new ServiceyesFragment());
...
//		pDected.setText("已发货");
//		jingping.setText("待收货");
//		yiwancheng.setText("待评价");
		healthPedia.setText("待付款");
		pDected.setText("已付款");
...
//		button.setOnClickListener(new MyOnItemClickListener());
	...
	private void InitImageView() {
		...
		bmpW = BitmapFactory.decodeResource(getResources(),
				R.drawable.tupian).getWidth();// 获取图片宽度
		DisplayMetrics dm = new DisplayMetrics();
		getWindowManager().getDefaultDisplay().getMetrics(dm);
		int screenW = dm.widthPixels;// 获取分辨率宽度
		
		offset = (screenW / pageSize - bmpW) / 4;// 计算偏移量--(屏幕宽度/页卡总数-图片实际宽度)/2
													// = 偏移量
