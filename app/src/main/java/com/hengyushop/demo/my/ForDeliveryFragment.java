 ...
 *
 */
public class ForDeliveryFragment extends Fragment implements OnClickListener{
	...
	private void load_list(boolean flag) {
		...
		try {
			...
			AsyncHttp.get(RealmName.REALM_NAME_LL+ "/get_order_page_size_list?user_id="+user_id+"" +
							"&page_size=10&page_index=1&strwhere=payment_status=2%20and%20express_status=1&orderby=",
	...
	private void load_list2(boolean flag) {
		...
		AsyncHttp.get(RealmName.REALM_NAME_LL+ "/get_order_page_size_list?user_id="+user_id+"" +
						"&page_size=10&page_index=1&strwhere=payment_status = 2 and express_status = 1&orderby=",
				new AsyncHttpResponseHandler() {
					@Override
					public void onSuccess(int arg0, String arg1) {
