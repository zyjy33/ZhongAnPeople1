...
public class JuLogAdapter extends BaseAdapter {
	private Context context;
	private ArrayList<Lglottery_Log> lists;
	private ImageLoader imageLoader;

	public JuLogAdapter(Context context, ArrayList<Lglottery_Log> lists,
	...
	public View getView(int position, View view, ViewGroup arg2) {
		...
		holder.lglottery_id.setText(lists.get(position).getId());
		holder.lglottery_name.setText(lists.get(position).getName());
		holder.lglottery_class.setText(lists.get(position).getType());
		/*
		 * String status = lists.get(position).getStatus(); switch
		 * (Integer.parseInt(status)) { case 0:
		 * holder.lglottery_class.setText("未结束"); break; case 1:
		 * holder.lglottery_name.setText(lists.get(position).getName());
		 * imageLoader.displayImage(U.IP + lists.get(position).getImg(),
		 * holder.lglottery_img); holder.lglottery_class.setText("已结束"); break;
		 * default: break; }
		 */
