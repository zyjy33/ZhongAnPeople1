...
public class WareInformationActivity extends BaseActivity implements
		...
		try {
			...
			if (status.equals("y")) {
				...
				if (list_tp.size() > 0) {
					...
					mAq.id(ling_tip).image(RealmName.REALM_NAME_HTTP+imgs_url);
				}
	...
	private void softshareWxChat(String text) {
		try {
			...
			api = WXAPIFactory.createWXAPI(WareInformationActivity.this, Constant.APP_ID, false);
						...
						public void onClick(View arg0) {
							...
							if (!user_name.equals("")) {
								if (user_point.equals("")) {
									Toast.makeText(WareInformationActivity.this, "请重新登录获取用户福利", 200).show();
								}else {
									try {
										...
										if (dq_jubi >= jubi){
											...
											Toast.makeText(WareInformationActivity.this, "您当前的福利不够兑换", 200).show();
										}
	...
	private void loadgouwuche() {
		try {
			...
			System.out.println("=====goods_id====================="+goods_id);
			AsyncHttp.get(RealmName.REALM_NAME_LL+ "/add_shopping_buy?user_id="+user_id+"&user_name="+user_name_key+
			...
			case R.id.btn_collect://收藏
				...
				if (!nickname.equals("")) {
					if (!user_name.equals("")) {
						...
						//					System.out.println("2================"+user_name);
						AsyncHttp.get(RealmName.REALM_NAME_LL+ "/user_favorite?article_id="+article_id+"&goods_id="+goods_id+"&user_name="+user_name_key+"" +
					...
					if (user_name.equals("")) {
						...
						AsyncHttp.get(RealmName.REALM_NAME_LL+ "/user_favorite_goods?article_id="+article_id+"&goods_id="+goods_id+"&user_name="+user_name_key+"" +
										"&user_id="+user_id+"&tags=",
								new AsyncHttpResponseHandler() {
									@Override
									public void onSuccess(int arg0, String arg1) {
