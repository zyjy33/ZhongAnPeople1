...
public class WareInformationActivityl extends BaseActivity implements
		OnClickListener {
	private List<Map<String, String>> allGriddatas = null;
	private TextView tv_market_money, tv_ware_name, tv_hengyu_money,
			tv_title_name, tv_ware_market_jifen, tv_integral, tv_spec_text,
			tv_wenzi1, tv_wenzi2;
	private Button btn_add_shop_cart;
	private ImageButton btn_ware_infromation_share, btn_wechat, btn_wx_friend,
			sms;
	...
	public static String sp_id, proFaceImg, proInverseImg, proDoDetailImg,
			proDesignImg, proName, proTip, retailPrice, marketPrice,
			proSupplementImg, goods_price, price, proComputerInfo, yth, key,
			releaseBossUid, AvailableJuHongBao, Atv_integral, productCount,
			title_ll, spec_ids, article_id, goods_id, subtitle, spec_text,
			point_id;
	XiangqingData xqdata;
	private String title, sub_title, sell_price, market_price, cost_price,
			rebate_price;
	...
	//
	// list_id.clear();
	// System.out.println("11购物清单的个数================"+list_id.size());
	// }
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.shenghuoguan_home);// new_ware_infromation
		// shenghuoguan_home
		...
		// progress = new DialogProgress(WareInformationActivityl.this);
	...
	private void con(final int index, int type) {
		try {
			...
			String data = proName
					+ "http://183.62.138.31:1010/mobile/goods/show-" + sp_id
					+ ".html?user_id=" + user_id + "&user_name=" + user_name
					+ "";
			// String data =
			// proName+"http://183.62.138.31:1010/mobile/goods/show-"+sp_id+".html?user_id=19&user_name=13714758507";
	...
	private void loadJuDuiHuan(String jdh_id) {
		AsyncHttp.get(RealmName.REALM_NAME_LL + "/get_game_point_content?id="
				+ jdh_id + "", new AsyncHttpResponseHandler() {
			@Override
	...
	private void formaJuDuiHuan(String result) {
		...
		try {
			...
			if (status.equals("y")) {
				...
				retailPrice = jobt.getString("point");
				...
				progress.CloseProgress();
				Toast.makeText(WareInformationActivityl.this, info, 200).show();
			}
		...
		try {
			...
			if (status.equals("y")) {
				...
				for (int i = 0; i < jsonay.length(); i++) {
					...
					System.out.println("=========数据article_id============"
							+ article_id);
				}
			...
			switch (msg.what) {
				case 0:
					...
					for (int i = 0; i < tempss.size(); i++) {
						urls.add(RealmName.REALM_NAME_HTTP
								+ tempss.get(i).getOriginal_path());
						System.out.println("tempss================="
								+ tempss.get(i).getOriginal_path());
					}

					if (urls.size() == 1) {
						mAq.id(ling_tip).image(
								RealmName.REALM_NAME_HTTP + proInverseImg);
					} else {
						market_information_images.setData(urls,
								new MyPosterOnClick() {
									@Override
	...
	private void softshareWxChat(String text) {
		try {
			...
			api = WXAPIFactory.createWXAPI(WareInformationActivityl.this,
			...
			switch (msg.what) {
				...
				case 4:
					...
					if (spjs == 1) {
						...
						webview.loadUrl("http://mobile.zams.cn/goods/conent-"
								+ article_id + ".html");
					}
					market_information_tip.setText(subtitle);

					tv_hengyu_money.setText(retailPrice + "分");
					tv_ware_market_jifen.setText("￥" + goods_price);
					tv_ware_market_jifen.getPaint().setFlags(
					...
					System.out.println("proInverseImg==================="
							+ proInverseImg);
					mAq.id(ling_tip).image(
							RealmName.REALM_NAME_HTTP + proInverseImg);
											...
											try {
												...
												if (!price.equals("0")) {
													Intent intent = new Intent(
															WareInformationActivityl.this,
															MyOrderConfrimActivity.class);
													intent.putExtra("200", "200");
				...
				case 2:
					try {
						...
						// //动态广告
						// if(images_ll.size()==1){
						// market_information_images.setData(images_ll,new
						// MyPosterOnClick() {
						//
						// @Override
						// public void onMyclick(int position) {
				...
				case 1:
					...
					setResult(3, new Intent(WareInformationActivityl.this,
							MainFragment.class));
					AppManager.getAppManager().finishActivity();
					break;
				case 0:
				...
				case 5:
					// adapter = new
					// GuigeListAdapter(WareInformationActivity.this,list,data);
	...
	private void loadgouwuche() {
		try {
			...
			System.out
					.println("=====user_id=====================" + article_id);
			System.out
					.println("=====user_name=====================" + goods_id);
						...
						public void onSuccess(int arg0, String arg1) {
							...
							try {
								...
								if (status.equals("y")) {
									...
									// Toast.makeText(WareInformationActivity.this,
									// info, 200).show();
									Intent intent = new Intent(
											WareInformationActivityl.this,
											MyOrderConfrimActivity.class);
	...
	private void innidata() {
		...
		img_shared.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				try {
					if (!nickname.equals("")) {
						if (!user_name.equals("")) {
							Intent intentll = new Intent(
									WareInformationActivityl.this,
									DBFengXiangActivity.class);
		...
		images_layout.setLayoutParams(new LinearLayout.LayoutParams(
		...
		market_information_describe.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				// Intent intent = new
				// Intent(WareInformationActivity.this,WareInformationDetailsActivity.class);
				// Bundle bundle = new Bundle();
				// bundle.putInt("wareid",
				// getIntent().getIntE-------------xtra("id", -1));
				...
				// Intent intent= new
				// Intent(WareInformationActivity.this,Webview1.class);
		...
		btn_add_shop_cart.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				// if (!user_name.equals("")) {
				//
				// CommomConfrim.showSheet(WareInformationActivity.this, new
				// onDeleteSelect() {
				// @Override
				...
				// } else {
				// Intent intent = new
				// Intent(WareInformationActivity.this,UserLoginActivity.class);
				// startActivity(intent);
				// progress.CloseProgress();
				// }
				//
		...
		market_information_sep.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				Intent intent = new Intent(WareInformationActivityl.this,
						SYBActivity.class);
	...
	private ArrayList<String> getDatall() {
		...
		if (!"".equals(proFaceImg)) {
			list.add(RealmName.REALM_NAME_HTTP + proFaceImg);
		}
		if (!"".equals(proInverseImg)) {
			list.add(RealmName.REALM_NAME_HTTP + proInverseImg);
		}
			...
			case R.id.bt_cart_low:
				try {
					...
					show_cart_low.setBackgroundColor(getResources().getColor(
							R.color.hongse));
					if (len > 0) {
						GoodsGgcsListAdapter ggcsadapter = new GoodsGgcsListAdapter(
								list_ggcs, getApplicationContext());
						new_list.setAdapter(ggcsadapter);
						setListViewHeightBasedOnChildren(new_list);
					} else {
						// loadguigecanshu();
						Toast.makeText(WareInformationActivityl.this, "无规格参数", 200)
								.show();
					}
				} catch (Exception e) {
			...
			case R.id.btn_collect:// 收藏
				...
				if (!nickname.equals("")) {
					if (!user_name.equals("")) {
						...
						AsyncHttp.get(RealmName.REALM_NAME_LL
								+ "/user_favorite?article_id=" + article_id
								+ "&user_name=" + user_name + "" + "&user_id=" + id
								+ "&tags=", new AsyncHttpResponseHandler() {
							@Override
							public void onSuccess(int arg0, String arg1) {
	...
	private void shangpingjieshan() {
		// webview.loadUrl("http://183.62.138.31:1010/mobile/goods/conent-"+article_id+".html");//商品介绍
		webview.loadUrl("http://mobile.zams.cn/goods/conent-" + article_id
				+ ".html");
	}
	...
	private void loadguigecanshu() {
		...
		String article_id = WareInformationActivityl.article_id;
		System.out.println("article_id==========================" + article_id);
		// AsyncHttp.get(RealmName.REALM_NAME_LL+ "/get_article_spec_list?"
		// +"channel_name=goods",
		AsyncHttp.get(RealmName.REALM_NAME_LL + "/get_spec_list?"
				...
				new AsyncHttpResponseHandler() {
					...
					public void onSuccess(int arg0, String arg1) {
						...
						try {
							...
							// CanshuGuiGeAdapter jysadapter = new
							// CanshuGuiGeAdapter(list,lists_ll,getApplicationContext());
							// new_list.setAdapter(jysadapter);
							// shangpingcsAdapter jysadapter = new
							// shangpingcsAdapter(list,data,getApplicationContext());
							// new_list.setAdapter(jysadapter);
							System.out.println("=====值1=====================");
							// WideMarketAdapter adapter = new
							// WideMarketAdapter(list,getApplicationContext(),
							// handler);
