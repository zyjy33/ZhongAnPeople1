...
public class WareInformationDetailsActivity extends BaseActivity {

	private int wareid;
	private WebView tv_detail;
	private String strUrl;
	private String ware_details;
		...
		public void dispatchMessage(Message msg) {
			switch (msg.what) {
				case 0:
					...
					tv_detail.getSettings().setLayoutAlgorithm(
							LayoutAlgorithm.SINGLE_COLUMN);
					tv_detail.getSettings().setLoadWithOverviewMode(true);
					// tv_detail.loadData(details, "text/html", "UTF-8");

					Document doc_Dis = Jsoup.parse(details);
					...
					tv_detail.loadDataWithBaseURL("", newHtmlContent, "text/html",
							"UTF-8", null);
					break;
				default:
					break;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tv_detail = (WebView) findViewById(R.id.tv_details);
		WebSettings webSettings = tv_detail.getSettings();
	...
	private void parse(String st) {
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				// ware_details =
				// URLDecoder.decode(object.getString("proComputerInfo2"),
				// "UTF-8");
				ware_details = object.getString("proComputerInfo");
				Message message = new Message();
				message.what = 0;
				message.obj = ware_details;
