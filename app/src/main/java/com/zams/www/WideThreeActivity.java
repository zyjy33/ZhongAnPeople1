...
public class WideThreeActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		progress = new DialogProgress(WideThreeActivity.this);
		progress.CreateProgress();
		Map<String, String> params = new HashMap<String, String>();
		params.put("act", "ProductItemsInfo");
		params.put("yth", "test");
		params.put("key", "test");
		params.put("startIndex", "0");
		params.put("count", "15");
		params.put("orderWhat", "");
		params.put("cityCode", city);
		params.put("ProductTypeId3", id+"");
		...
		new Thread() {
			public void run() {
				List<WareData> allnames = waredao.findAllWare1(id);
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				System.out.println(typename + "id:" + id);
	...
	private void parse(String st) {
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				data.productItemId = object.getInt("id");
