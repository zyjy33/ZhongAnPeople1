 ...
 *
 */
public class MyJuTuanOrderXqAdapter extends BaseAdapter {
	...
	public MyJuTuanOrderXqAdapter(Context context, List<MyOrderData> list, Handler handler) {
		this.list = list;
		...
		if (convertView == null) {
			try {
				...
				for (int i = 0; i < list.get(position).getList().size(); i++) {
					...
					//			holder.tv_chengyuan = (TextView) vi.findViewById(R.id.tv_chengyuan);
					...
					//			holder.tv_market_price.setText("市场价:￥"+list.get(position).getList().get(i).getMarket_price());
					...
					mAq.id(holder.tupian).image(RealmName.REALM_NAME_HTTP+list.get(position).getList().get(i).getGroupon_img_url());


					try {

						//				holder.tv_chengyuan.setVisibility(View.VISIBLE);

						/**
						 * 查看成员
						 */
						//				holder.tv_chengyuan.setOnClickListener(new OnClickListener() {
						//
						//					@Override
						//					public void onClick(View v) {
						//						try {
						//							  Intent intent3 = new Intent(context, Webview1.class);
						//							  intent3.putExtra("ct_id",list.get(position).getList().get(p).getOrder_id());
