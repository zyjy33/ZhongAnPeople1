 ...
 *
 */
public class ScrollTopViewll extends LinearLayout {
	...
	public ScrollTopViewll(Context context) {
		super(context);
		init();
	}

	public ScrollTopViewll(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public void setData(List<BeanVo> articleList) {
		...
		if (articleList != null) {
			...
			if (articleList.size() > 2) {
				getLayoutParams().height = Utils.dip2px(20); // 调节滚动数据的高度
				// 滚动
				cancelAuto();
	...
	private void addContentView(int position) {
		...
		// "恭喜“周星星” 抢到中国移动话费100元充值卷 "
		mHolder.nameTv.setText("恭喜" + "“" + article.user_name + "”用户,抢到"
				+ article.groupon_title);
		// }else {
